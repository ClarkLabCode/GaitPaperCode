function [p_ctrl, h_ctrl, stats_ctrl, p_exp, h_exp, stats_exp, h, p, ks2stat] = PlotMoonwalkerGaitFigures( newData, lineThickness, suppressPlots, num_bootstraps )
% This function generates all the plots associated with the Moonwalker data
% for the gait paper
...
[ p_ctrl, h_ctrl, stats_ctrl, data_ctrl, time_ctrl, meanSpeed_ctrl, errorSpeed_ci_ctrl, normMeanSpeed_ctrl, errorNormSpeed_ci_ctrl, ...
    ...
    = PlotOptoStanceIncreaseAllLimbs( newData, 'Control', 'Moonwalker', 1, suppressPlots, num_bootstraps);
...
[ p_exp, h_exp, stats_exp, data_exp, time_exp, meanSpeed_exp, errorSpeed_ci_exp, normMeanSpeed_exp, errorNormSpeed_ci_exp, ...
    ...
    = PlotOptoStanceIncreaseAllLimbs( newData, 'Optogenetic', 'Moonwalker', 1, suppressPlots, num_bootstraps);
...
title({'Moonwalker', 'Unnormalized Walking Speed'});
xlabel('Time (ms)');
...
title({'Moonwalker', 'Normalized Walking Speed'});
xlabel('Time (ms)');
ylabel('Forward Speed (mm/s)');
ylim([0 2]);
xlim([min(time_ctrl), max(time_ctrl)]);
...
title({'Moonwalker','Stance Duration Distributions - All Limbs'});
...
title({'Moonwalker','Stance Duration Distributions - All Limbs',sprintf('p = %.4e',p),sprintf('D = %.4f',ks2stat)});
xlabel('Stance Duration');
ylabel('cdf');
% ylim([0 1]);
legend(legend_labels); 
