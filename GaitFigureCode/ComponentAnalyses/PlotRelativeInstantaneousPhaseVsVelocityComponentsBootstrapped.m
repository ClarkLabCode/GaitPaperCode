function PlotRelativeInstantaneousPhaseVsVelocityComponentsBootstrapped( newData, vfBinEdges, vrBinEdges, nbins, cmap, logscale, corder, linewidth, smoothing, showDistributions, suppressExtraPlots, num_bootstraps)
...
%% Calculate circular statistics binned by forward velocity

% Define the data to be included
X = [Phi_rel, vf_discrete];
X = X(~isnan(vf_discrete), :);
ID = ID(~isnan(vf_discrete));
...
%% Calculate bootstraps on the data of interest

% Define the data for sampling
Y = splitapply(@(x){x}, X, ID);

tic;
% Calculate the bootstrap confidence intervals
ci = bootci(num_bootstraps, @(x)local_bootfun(x), Y);
toc;

% Split the CI into upper and lower bounds
lower_ci = squeeze(ci(1,:,:));
upper_ci = squeeze(ci(2,:,:));

clearvars X M;

%% Plot circular statistics (with bootstrapped confidence intervals) binned by forward velocity
...
subplot(1,2,2);
hold on;
PlotConfidenceIntervalWithErrorPatch( v_bins, sd_delta_phi, lower_ci(:,7:12), upper_ci(:,7:12), corder);
xlabel('v_{||} (mm/s)');
...
function [ M ] = local_bootfun(Y)
% This function computes the bootstrapped output of limb relative phases

% Convert to an array
X = cell2mat(Y);

% Define the anonymous function
statfun = @(x) [mod(circ_mean(x(:, 1:6),[],1),(2*pi))/(2*pi), circ_std(x(:, 1:6), [],[], 1)/(2*pi), unique(x(:,end))];
