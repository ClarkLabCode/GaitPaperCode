function PlotRelativeInstantaneousPhasePolarHistograms( newData, nbins, corder_pairwise, corder_all, rlims, vfBinEdges, smoothing, suppressExtraPlots )
...
% Get the bin edges
binEdges = linspace(0, 2*pi, nbins);
...
N = zeros(length(binCenters), size(Phi_rel, 2));

for ind=1:size(Phi_rel,2)
    N(:,ind) = histcounts(Phi_rel(:,ind), binEdges, 'normalization','pdf');
end
...
polarplot([binCenters, binCenters(1)], [N; N(1,:)], 'linewidth', 2);
...
if ~ suppressExtraPlots
    ...
    % Compute histograms
    N = zeros(length(binCenters), size(Phi_rel,2), length(vfBinEdges) + 1);
    
    % v < vfBinEdges(1) mm/s
    idx = v < vfBinEdges(1);
    for ind=1:size(Phi_rel,2)
        N(:, ind, 1) = histcounts(Phi_rel(idx,ind), binEdges, 'normalization','pdf');
    end
    
    % v > vfBinEdges(end) mm/s
    idx = v > vfBinEdges(end);
    for ind=1:size(Phi_rel,2)
        N(:, ind, end) = histcounts(Phi_rel(idx,ind), binEdges, 'normalization','pdf');
    end
    ...
    for ind = 2:length(vfBinEdges)
        ...
        for ii=1:size(Phi_rel,2)
            N(:, ii, ind) = histcounts(Phi_rel(idx,ii), binEdges, 'normalization','pdf');
        end
    end
    ...
    for ind=1:size(N,3)
        ...
        polarplot([binCenters, binCenters(1)], [squeeze(N(:,:,ind)); squeeze(N(1,:, ind))], 'linewidth', 2);
...
if ~ suppressExtraPlots
    ...
    idx = 1:6;
    N = zeros(length(binCenters), 5, 6);
    for ind=1:6
        Phi_rel = mod(Phi(:, idx(idx~=ind) ) - Phi(:, ind), 2*pi);
        for ii=1:5
            N(:, ii, ind) = histcounts(Phi_rel(:, ii), binEdges, 'normalization','pdf');
        end
    end
